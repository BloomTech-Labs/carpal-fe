...
describe("login user", () => {
    test("Renders Login component/Testing basic JSx Elements", () => {
        ...
        expect(wrapper.queryByPlaceholderText(/Password/i).nodeName).toEqual('INPUT')
        expect(wrapper.queryByPlaceholderText(/Email@email.com/i).nodeName).toEqual('INPUT')
        expect(wrapper.getByText("Login With Google")).not.toBeNull()
        expect(wrapper.getAllByText(/Login/i)[0].textContent).toBeTruthy()
        expect(wrapper.queryByText('New to the website?').childNodes[1].nodeName).not.toBeUndefined()
        expect(wrapper.queryByText('New to the website?').childNodes[1].textContent).toEqual('Sign Up!')
        expect(2).toBe(2);
    });
    ...
    test('should LogIn a user', async () => {
        ...
        await wait(()=>{
            ...
            // expect(actions.LogInAction).toHaveBeenCalledWith({"email": "lesley@gmail.com", "password": "lesley"});
